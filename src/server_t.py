...
from helper_functions import measure_execution_time, load_test_files

my_lock = threading.Lock()
def handle_client(client_socket: socket.socket, list) -> None:
    from search import SearchAlgorithms
    """
    ...
    with my_lock:
        ...
        for file_reader in list:
            ...
            try:
                ...
                if file_reader.reread_on_query:
                    debug_log = f"DEBUG: The total execution time of {file_reader.path} in ms: {total + search_execution_time:.4f}" 
                else:
                    debug_log = f"DEBUG: The total execution time minus the time spent on sorting of {file_reader.path}, in ms: {file_load_execution_time + search_execution_time:.4f}\n" 
...
def main():
    ...
    files, ssl_settings = load_test_files()
    # print(files, ssl_settings)
    ...
    for file in files:
        # print(file)
        file_reader = FileReader(file, REREAD_ON_QUERY)
    ...
    try:
        while True:
            ...
            client_thread = threading.Thread(target=handle_client, args=(client_socket, file_readers))
